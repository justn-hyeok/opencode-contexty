...
2. **AASM** — 코드가 아니라 _의도_를 검사하는 능동형 에이전트.
3. **TLS** — 터미널 출력을 알아서 요약해주는 똑똑한 래퍼.
...
플러그인이 로그 모으면, 확장이 _보여주고_ _관리_할 수 있게 해줍니다.
...
| 명령어 | 설명 |
| ---------------- | ----------------------------------------- |
| `/agent active` | 감독 켜기 (기본) |
| `/agent passive` | 감독 끄기 |
| `/agent status` | 지금 모드 보기 |

### HSCMM: 자동 컨텍스트 수집

HSCMM은 뒤에서 조용히 돌아갑니다:

1. **도구 쓸 때마다** `.contexty/tool-parts.json`에 기록됨
2. **VSCode 확장**이 이거 읽어서 보여줌
3. **명령어 칠 필요 없이 확장 UI로 관리**

명령어 필요없어요. 플러그인이 모으고, 확장이 보여주고, 당신이 제어합니다.
...
1. **알림 뜸** — 뭐가 문제인지 알려줌
2. **요청 차단됨** — 시스템 메시지로 대체
3. **AI가 설명** — 왜 안 되는지 알려줌
4. **선택은 당신**: 다시 쓰거나 AASM 끄거나

예시:

```
당신: "코드 다 main.ts 하나에 때려박아"
...
AASM이 그 도구예요. 세미콜론 체크하는 린터 아니고요. 코드 짜기 전에 _의도_부터 검토하는 **시니어 개발자**입니다.
